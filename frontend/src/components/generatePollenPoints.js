...
export async function generatePollenPointsBySuburbs(suburbNames) {
  ...
  const tasks = suburbNames.map(({ suburbName,pollen }) =>
    limitConcurrency(() => {
      ...
      return new Promise((resolve) => {
        ...
        for (let i = 0; i < numWorkers; i++) {
          ...
          worker.postMessage({ feature, numPoints: pointsPerWorker,pollenData:pollen });
          ...
          worker.onmessage = (event) => {
            ...
            if (finishedWorkers === numWorkers) {
              ...
              allPointsBySuburb[suburbName] = { pollengeojson: pollenGeoJSON,feature:feature,pollen:pollen };
